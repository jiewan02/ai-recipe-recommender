...
model = AutoModelForCausalLM.from_pretrained(
    ...
    # device_map="auto",      # accelerate가 알아서 3개 GPU에 분산
    device_map={"": 1},       # 1번 GPU에 올려서 돌리는 설정
...
11) difficulty
사용자가 아래 표현 중 하나라도 포함하면 반드시 difficulty 배열에 해당 표준 난이도를 넣으십시오.

● "쉬운", "간단한", "쉽게", "초보자", "초보용", "입문자"  
    → difficulty: ["초급"]
...
● "어려운", "손이 많이 가는", "고급요리", "정성 들어가는"  
    → difficulty: ["고급"]

● "전문가용", "매우 어려운", "신의 경지", "하드코어 요리"  
    → difficulty: ["신의경지"]

⚠ 규칙:
- 표현이 조금이라도 포함되어 있으면 반드시 매핑해야 합니다.
- difficulty 배열은 최소 1개 이상이어야 하며, 중복 제거합니다.
- 사용자가 둘 이상의 표현을 동시에 쓰면 여러 난이도를 모두 포함해도 됩니다.
